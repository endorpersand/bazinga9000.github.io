function englishOre() {
    const initials = 'bcdfghjklmnprstvwxyz'.split('').concat(['bl', 'br', 'ch', 'cl', 'cr', 'dr', 'fn', 'fl', 'fr', 'gl', 'gr', 'ph', 'pl',
        'pr', 'sh', 'sk', 'sl', 'sm', 'sn', 'sp', 'spl', 'spr', 'st', 'str', 'sw', 'qu', 'th', 'tj', 'tr', 'tw', ''
    ]);

    const nuclei = ['a', 'e', 'i', 'o', 'u', 'ai', 'ay', 'ee', 'ea', 'y', 'ie', 'oa', 'ew'];

    const finals = 'bcdfgklmnprstvxz'.split('').concat(['ch', 'ck', 'ct', 'ft', 'ld', 'lg', 'lk', 'lt', 'mp', 'nd', 'ng', 'nk', 'nt', 'ph', 'pt',
        'rk', 'rt', 'sh', 'sk', 'sp', 'ss', 'st', 'th', ''
    ]);

    const suffixes = ['ite', 'ium', 'ane', 'ine', 'ene', 'ate', 'lite', 'lith'];
    ...
    const syllable_count = choose([1, 1, 1, 2, 2, 3]);
...
function chineseSyllable(neutral) {
    const cons = ['b', 'p', 'm', 'f', 'd', 't', 'n', 'l', 'g', 'k', 'h', 'zh', 'ch', 'sh', 'r', 'z', 'c', 's', ''];
    const consI = ['b', 'p', 'm', 'd', 't', 'n', 'l', 'j', 'q', 'x', ''];
    const consY = ['n', 'l', 'j', 'q', 'x', ''];
    const consIsp = ['zh', 'ch', 'sh', 'r', 'z', 'c', 's'];
    const vowel = ['e', 'ei', 'ou', 'en', 'eng', 'a', 'ai', 'ao', 'an', 'ang', 'wu', 'wo', 'wei', 'wen', 'weng', 'wa', 'wai', 'wan', 'wang', 'yi', 'ye', 'you', 'yin', 'ying', 'ya', 'yao', 'yan', 'yang', 'yu', 'yue', 'yun', 'yong', 'yuan', 'er'];
    const vowelc = ['e', 'ei', 'ou', 'en', 'eng', 'a', 'ai', 'ao', 'an', 'ang', 'u', 'uo', 'ui', 'un', 'ong', 'ua', 'uai', 'uan', 'uang', 'i', 'ie', 'iu', 'in', 'ing', 'ia', 'iao', 'ian', 'iang', 'ü', 'üe', 'ün', 'iong', 'üan', 'er'];
    ...
    const sel_tone = tones[Math.floor(Math.random() * tones.length)];
    syl = syl.replace(/!/, sel_tone);
    return syl;
}

function chineseOre() {
    const syllables = choose([1, 1, 2, 2, 2, 2, 2, 3, 3, 3]);
    ...
    const ore_suffixes = ['', '', 'shi\u0301', 'kua\u0300ng'];
...
function japaneseOnSyllable() {
    const vowel = ['a', 'i', 'u', 'e', 'o', 'ya', 'yu', 'yo'];
    const vowelnoy = ['a', 'i', 'u', 'e', 'o'];
    const cons = ['k', 'g', 's', 'z', 't', 'd', 'n', 'h', 'b', 'm', 'r', ''];
    const lvowel = ['ai', 'uu', 'ei', 'ou', 'yuu', 'you'];
    const final = ['k', 't'];
...
function japaneseOnOre() {
    ...
    const ore_suffixes = ['', '', 'kou', 'seki'];
    word = word + ore_suffixes[Math.floor(Math.random() * ore_suffixes.length)];

    word = word.replace(/n;(?=[aiueoy])/, 'n\'');
    word = word.replace(/n;h/g, 'nb');
    word = word.replace(/n;/g, 'n');
    word = word.replace(/k;t/g, 'tt');
    word = word.replace(/t;([kst])/g, '$1$1');
    word = word.replace(/t;h/g, 'pp');
...
function vietnameseOre() {
    ...
    let finals = ['', 'c', 'm', 'n', 'ng', 'p', 't']; // + ch, nh (for a, ê, i)
    const monophs = [
    ...
    const peek = iter => iter[iter.length - 1];
    const diphables = char => {
        let canDiph = [];
    ...
    const syllables = [];
    const sylcount = choose([1, 1, 1, 2, 2, 3]);
    for (let i = 0; i < sylcount; i++) {
        const initial = choose(initials);

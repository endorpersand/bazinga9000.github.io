...
function getComparisonImage(name) {
    ...
    img.setAttribute('src', '/resources/randomore/comparisons/' + name + '.png');
    return img;
}
...
function createSvg(type, quarter, size) {
    ...
    switch (quarter) {
        case 0:
            src = '/resources/randomore/comparisons/' + type + '.svg';
            break;
        case 1:
            src = '/resources/randomore/comparisons/onequarter' + type + '.svg';
            break;
        case 2:
            src = '/resources/randomore/comparisons/half' + type + '.svg';
            break;
        case 3:
            src = '/resources/randomore/comparisons/threequarter' + type + '.svg';
            break;
    }
...
function drawComparisonGraph(canvas, customOreName, customOreColor, comparisons, itemCanvas) {
    ...
    for (i of comparisons) {
        ...
        if (i[0] != customOreName) {
            const img = document.createElement('img');
            img.setAttribute('src', '/resources/randomore/comparisons/' + i[0] + '.png');
            img.n = n;
            img.onload = function() {
...
function generateOre() {
    ...
    const blockimage = '/resources/randomore/block/' + choose(blocks) + '.png';
    ...
    loadFlag = false;
    functions = [];
    const itemimage = '/resources/randomore/item/' + choose(items) + '.png';
    ...
    const oreimage = '/resources/randomore/ore/' + choose(ores) + '.png';

    const spawnblock = choose(Object.keys(spawn_rarities));
    const baseimage = '/resources/randomore/ore_block/' + spawnblock + '.png';
    ...
    mimg.src = `/resources/randomore/comparisons/${pickaxeBlocks.includes(spawnblock) ? 'pickaxe' : 'shovel'}.png`;
	...
	*/
    ...
    if (canMakeArmor) {
        ...
        for (i of [
                ['Helmet', prot_helmet],
                ['Chestplate', prot_chest],
                ['Leggings', prot_leggings],
                ['Boots', prot_boots]
            ]) {
		...
		*/
        ...
        for (i of [
                ...
                ['Chestplate', chestplateDurability],
                ['Leggings', leggingsDurability],
